...
- 이미지: GPT 시트 4장 `assets/characters/{ria,prince}/_concept/`, `assets/characters/_concept/cast_sheet.png`.
  시트에서 잘라 넣음(rembg `isnet-anime` 로 배경 제거): 초상 5 + 리아 표정 6 + 의상 전신 7(c001~c007) + hero 4. `python tools/check_assets.py` → 배경 3장만 없음.
...
2. 배경 3장(plaza_night / cafe / garden, 720×1280) GPT 생성 → `assets/backgrounds/`. 프롬프트는 `assets/README.md`.
3. Stage 4~6 시드 작성(`content/seeds/`), c006/c007 을 선택지에 넣기.
